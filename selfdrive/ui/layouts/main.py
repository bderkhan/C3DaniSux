...
OFFROAD_TRANSITION_SECONDS = 0.18
...
EDGE_SWIPE_ZONE = 32                 # px from the left edge a swipe-back must start within
...
class MainLayout(Widget):
  ...
  def _render_transition(self, content_rect: rl.Rectangle) -> bool:
    ...
    from_rect = rl.Rectangle(round(content_rect.x + from_x), content_rect.y, content_rect.width, content_rect.height)
    to_rect = rl.Rectangle(round(content_rect.x + to_x), content_rect.y, content_rect.width, content_rect.height)

    rl.draw_rectangle_rec(content_rect, TRANSITION_SURFACE_BG)
    if self._transition.state == MenuTransitionState.PUSHING:
      self._render_layout_surface_translated(self._layouts[from_mode], content_rect, from_x)
      if from_dim > 0:
        rl.draw_rectangle_rec(from_rect, rl.Color(0, 0, 0, from_dim))
      self._render_layout_surface_translated(self._layouts[to_mode], content_rect, to_x)
    else:
      self._render_layout_surface_translated(self._layouts[to_mode], content_rect, to_x)
      if to_dim > 0:
