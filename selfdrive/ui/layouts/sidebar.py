...
class Sidebar(Widget):
  ...
  def _draw_buttons(self, rect: rl.Rectangle):
    ...
    if self._recording_audio:
      self._mic_indicator_rect = rl.Rectangle(rect.x + rect.width - 106, rect.y + 240, 70, 38)
  ...
  def _draw_network_indicator(self, rect: rl.Rectangle):
    ...
    if self._net_type in (NETWORK_TYPES[NetworkType.wifi], NETWORK_TYPES[NetworkType.ethernet]):
      content_w += 26 + self._icon_wifi.width
    ...
    if self._net_type in (NETWORK_TYPES[NetworkType.wifi], NETWORK_TYPES[NetworkType.ethernet]):
      wifi_x = text_x + text_size.x + 26
      wifi_y = NETWORK_RECT.y + (NETWORK_RECT.height - self._icon_wifi.height) / 2
      rl.draw_texture(self._icon_wifi, int(wifi_x), int(wifi_y), Colors.WHITE)
