...
from openpilot.iqpilot.ui.layouts.settings.settings import IQSettingsLayout
...
EDGE_SWIPE_ZONE = 32           # px from the left edge a swipe-back touch must start within
...
SETTINGS_TRANSITION_SECONDS = 0.16
...
class SettingsHubLayout(Widget):
  ...
  def _render_transition(self, rect: rl.Rectangle) -> bool:
    ...
    if self._transition.state == MenuTransitionState.PUSHING:
      from_x = _lerp(0.0, -screen_width, e)
      to_x = from_x + screen_width
      from_dim = int((1.0 - _lerp(1.0, 0.80, e)) * 255)
      to_dim = 0
    else:
      from_x = _lerp(0.0, screen_width, e)
      to_x = from_x - screen_width
      from_dim = 0
      to_dim = int((1.0 - _lerp(0.80, 1.0, e)) * 255)

    from_rect = rl.Rectangle(round(rect.x + from_x), rect.y, rect.width, rect.height)
    to_rect = rl.Rectangle(round(rect.x + to_x), rect.y, rect.width, rect.height)

    rl.draw_rectangle_rec(rect, TRANSITION_SURFACE_BG)
    if self._transition.state == MenuTransitionState.PUSHING:
      self._render_mode_surface_translated(from_mode, rect, from_x)
      if from_dim > 0:
        rl.draw_rectangle_rec(from_rect, rl.Color(0, 0, 0, from_dim))
      self._render_mode_surface_translated(to_mode, rect, to_x)
    else:
      self._render_mode_surface_translated(to_mode, rect, to_x)
      if to_dim > 0:
        rl.draw_rectangle_rec(to_rect, rl.Color(0, 0, 0, to_dim))
      self._render_mode_surface_translated(from_mode, rect, from_x)
      if from_dim > 0:
        rl.draw_rectangle_rec(from_rect, rl.Color(0, 0, 0, from_dim))
    return True
  ...
  def _render_grid(self, rect: rl.Rectangle, interactive: bool = True):
    ...
    # Grid landing
    title = tr("Settings")
    title_offset = 4 * BUBBLE_SIZE + 3 * 18 + 36
