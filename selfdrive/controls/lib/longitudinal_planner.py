...
class LongitudinalPlanner(LongitudinalPlannerIQ):
  ...
  def update(self, sm):
    ...
    self.mpc.update(sm['modelV2'], sm['radarState'], personality=sm['selfdriveState'].personality)
    ...
    cruise_limited = self.a_cruise < output_a_target_mpc
    if cruise_limited:
      output_a_target_mpc = self.a_cruise
    if cruise_limited or not self.mpc.status:
