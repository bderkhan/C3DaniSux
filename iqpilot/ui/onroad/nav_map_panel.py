...
class MapboxTileProvider:
  ...
  def _fetch_worker(self, tile_key: tuple[int, int, int], token: str) -> None:
    ...
    try:
      ...
      if response.status_code == 401 and self._style != "dark-v11":
        # some tokens can't access the navigation style ("Direct access not allowed")
        self._style = "dark-v11"
        url = build_mapbox_tile_url(z, x, y, tile_size=TILE_SIZE, scale=TILE_SCALE, style=self._style)
...
class OsmOfflineProvider:
  ...
  def _consume_blob(self, tile_key: tuple[int, int, int], payload: bytes) -> None:
    ...
    image = rl.load_image_from_memory(MapboxTileProvider._image_ext(payload), payload, len(payload))
    _downscale_tile_image(image)
    decode_us = (time.monotonic_ns() - started_ns) // 1000
    started_ns = time.monotonic_ns()
    texture = rl.load_texture_from_image(image)
    upload_us = (time.monotonic_ns() - started_ns) // 1000
    rl.unload_image(image)
    ...
    self._textures[tile_key] = texture
    total_us = decode_us + upload_us + unload_us
    if total_us >= NAV_TEXTURE_WARN_US or decode_us >= NAV_TILE_OP_WARN_US or upload_us >= NAV_TILE_OP_WARN_US:
      sample = PerfSample(
        texture_decode_us=int(decode_us),
        texture_upload_us=int(upload_us),
        texture_unload_us=int(unload_us),
        texture_consume_us=int(total_us),
        texture_batch_size=1,
      ...
      _emit_nav_perf(
        ...
        total_time_us=int(total_us),
        batch_size=1,
        detail=f"provider=offline decode_us={decode_us} upload_us={upload_us} unload_us={unload_us} bytes={len(payload)} cache_after={len(self._textures)}",
        sample=sample,
      )
  ...
  def release(self) -> None:
    """Free all GPU tile textures. See MapboxTileProvider.release()."""
    for texture in self._textures.values():
      rl.unload_texture(texture)
    self._textures.clear()
...
class NavMapPanel(Widget):
  ...
  def _draw_maneuver_icon(self, tile: rl.Rectangle):
    ...
    rl.draw_text_ex(
      self._micro_font,
      distance_text,
      rl.Vector2(tile.x + (tile.width - dist_width) * 0.5, tile.y + tile.height - 34),
      distance_size,
      0,
      rl.WHITE,
    )
