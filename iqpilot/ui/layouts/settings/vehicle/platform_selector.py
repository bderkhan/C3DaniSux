...
class LegendWidget(Widget):
  ...
  def _render(self, rect):
    ...
    for color, text in items:
      ...
      # Rounded color chip, vertically centered with the row text
      ts = measure_text_cached(font, f"- {text}", 40)
      row_cy = (y - 7) + ts.y / 2
      rl.draw_rectangle_rounded(rl.Rectangle(x, row_cy - 18, 36, 36), 0.45, 10, color)
      rl.draw_text_ex(font, f"- {text}", rl.Vector2(x + 56, y - 7), 40, 0, text_color)
      y += 50
