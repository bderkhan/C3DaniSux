...
from openpilot.system.ui.widgets.network import NetworkUI, PanelType, TEAL
...
class IQNetworkUI(NetworkUI):
  def __init__(self, wifi_manager):
    ...
    self.scan_button = Button(tr("Scan"), self._scan_clicked, button_style=ButtonStyle.TRANSPARENT_WHITE_TEXT, font_size=60, border_radius=30)
  ...
  def _run_scan(self):
    # Hold the scanning flag until networks come back (or it fails) so the spinner
    # stays in sync with the "Scanning..." button, not just the quick scan call.
    self._wifi_manager._scanning = True
    try:
      ...
      self._wifi_manager._scanning = False
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    if self._current_panel == PanelType.WIFI:
      ...
      bg = TEAL if self.scan_button.enabled else rl.Color(52, 55, 62, 255)
