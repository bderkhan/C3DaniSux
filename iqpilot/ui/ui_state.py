...
from openpilot.iqpilot.ui.layouts.settings.display import OnroadBrightness
...
class IQUIState:
  def __init__(self):
    self.params = Params()
    # NOTE: these are deserialized every UI frame; only subscribe to topics a rendered widget reads.
    # Dropped iqCarState/iqCarParams (zero readers) and rawAudioData (only feedbackd, a separate
    # process, reads it) to trim the per-frame SubMaster cost on the onroad hot path.
  ...
  def update_params(self) -> None:
    ...
    self.night_mode = self.params.get_bool("NightMode")
  ...
  def _compute_is_night(self) -> bool:
    # Deferred import: nav_helpers imports selfdrive.ui.ui_state (which subclasses this), so a
