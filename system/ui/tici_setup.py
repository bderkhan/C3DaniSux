...
class Setup(Widget):
  def __init__(self):
    ...
    self._ble_status_label = Label("", 44, FontWeight.NORMAL, rl.GuiTextAlignment.TEXT_ALIGN_CENTER,
                                   text_color=rl.Color(160, 160, 165, 255))
  ...
  def _ble_sync(self):
    ...
    if self._ble_pending_install_url is not None:
      ...
      if self.state not in (SetupState.DOWNLOADING, SetupState.DOWNLOAD_FAILED):
        self.download(url)
    # mirror on-screen install state to the phone (coarse progress)
    if self.state == SetupState.DOWNLOADING:
      self.ble_setup.set_install_progress("downloading", int(self.download_progress))
  ...
  def _render_ble_overlay(self, rect: rl.Rectangle):
    # Persistent, unobtrusive footer: shows the pairing code (so the user can
    # type it in the app) or the connected state. Hidden once installing.
    ...
    if snap.get("phone_active"):
      text = "Connected to your phone — continue setup there"
    else:
      text = f"Or set up from the konn3kt app · code {self.ble_setup.code}"
    self._ble_status_label.set_text(text)
    footer = rl.Rectangle(rect.x, rect.y + rect.height - 70, rect.width, 60)
    self._ble_status_label.render(footer)
