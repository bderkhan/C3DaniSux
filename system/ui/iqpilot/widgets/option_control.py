...
class IQOptionControl(ItemAction):
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    # background
    rl.draw_rectangle_rounded(self.container_rect, CONTAINER_ROUNDNESS, 20, style.OPTION_CONTROL_CONTAINER_BG)
  ...
  def _render_button(self, rect: rl.Rectangle, text: str, enabled: bool):
    ...
    is_pressed = (enabled and rl.check_collision_point_rec(mouse_pos, rect) and
    ...
    if not enabled:
      bg_color, glyph_color = BTN_BG_DISABLED, GLYPH_DISABLED
    elif is_pressed:
      bg_color, glyph_color = BTN_BG_PRESSED, GLYPH_PRESSED
    else:
      bg_color, glyph_color = BTN_BG, GLYPH_COLOR

    # Circular button
    rl.draw_circle(int(cx), int(cy), radius, bg_color)

    # Crisp drawn glyph (rounded bars) instead of a font character
    arm = radius * 0.46
    h_bar = rl.Rectangle(cx - arm, cy - GLYPH_THICK / 2, arm * 2, GLYPH_THICK)
    rl.draw_rectangle_rounded(h_bar, 1.0, 6, glyph_color)
    if text == "+":
      v_bar = rl.Rectangle(cx - GLYPH_THICK / 2, cy - arm, GLYPH_THICK, arm * 2)
      rl.draw_rectangle_rounded(v_bar, 1.0, 6, glyph_color)
